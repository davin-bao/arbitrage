...
class HistoricalMarketService(MarketService):
        ...
        for filename in os.listdir(self.csv_directory):
            if filename.endswith('.csv'):
                ...
                if len(parts) >= 3:
                    ...
                    pair = Pair(
                        symbol=symbol,
                        base=symbol.split('_')[0] if '_' in symbol else symbol.split('/')[0],
                        quote=symbol.split('_')[1] if '_' in symbol else symbol.split('/')[1],
                    ...
                    self._load_pair_data_from_csv(pair, file_path, long_exchange, short_exchange)
    
    def _load_pair_data_from_csv(self, pair: Pair, file_path: str, long_exchange: str, short_exchange: str):
        """
        从CSV文件加载特定交易对的历史数据
        """
        data_list = []
    ...
    def get_snapshot(self, pairs: List[Pair]) -> Dict[str, MarketSnapshot]:
        ...
        获取下一个市场快照
        """
        print(f"获取下一个市场快照: {self._timestamps[self._current_index]}")
        ...
        for pair in pairs:
            # 从文件名中获取long_exchange和short_exchange
            # 由于无法直接从Pair对象获取，我们需要从缓存中推断
            exchanges_in_cache = set()
            for key in self._data_cache[timestamp].keys():
                _, exchange = key.rsplit('_', 1)
                exchanges_in_cache.add(exchange)
            print(f"从缓存中推断交易所: {pair.symbol}")
            # 获取这个交易对的两条腿数据
            long_symbol = self._find_symbol_for_exchange(pair.symbol, list(exchanges_in_cache)[0] if exchanges_in_cache else "")
            short_symbol = self._find_symbol_for_exchange(pair.symbol, list(exchanges_in_cache)[1] if len(exchanges_in_cache) > 1 else "")
            
            # 使用更灵活的方式确定long和short交易所
            long_exchange = list(exchanges_in_cache)[0] if exchanges_in_cache else ""
            short_exchange = list(exchanges_in_cache)[1] if len(exchanges_in_cache) > 1 else ""
            
            long_key = f"{long_symbol}_{long_exchange}" if long_symbol else f"{pair.symbol}_{long_exchange}"
            short_key = f"{short_symbol}_{short_exchange}" if short_symbol else f"{pair.symbol}_{short_exchange}"
